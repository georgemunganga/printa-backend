...
func NewHandler(db *sql.DB, signatureKey string) *Handler {
	return &Handler{db: db, signatureKey: strings.TrimSpace(signatureKey)}
}

func NewHandlerFromEnv(db *sql.DB) *Handler {
	return NewHandler(db, os.Getenv("LENCO_WEBHOOK_SIGNATURE_KEY"))
}
